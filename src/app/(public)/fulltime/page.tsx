...
/**
 * 정규직 채용관 — 비자 필터 + 적격 뱃지 포함
 * Full-time job listing — with visa filter + eligibility badges
 */

import { useState, useEffect, useCallback } from 'react';
import { Search, MapPin, Eye, Users, ChevronLeft, ChevronRight, Briefcase, Shield, AlertTriangle, CheckCircle, XCircle } from 'lucide-react';
...
export default function FulltimePage() {
    ...
    fetch('/api/auth/profile-detail', { credentials: 'include' })
      ...
      .then(profile => {
        ...
        fetch('/api/visa-verification/me', { credentials: 'include' })
          ...
          .catch(() => { setUserMode('individual_unverified'); });
      })
      .catch(() => { setUserMode('guest'); });
  }, []);
  ...
  }, [page, keyword, visaFilter, subTypeFilter, userMode, eligibilityFilterOn]);
              ...
              {jobs.length === 0 && premiumJobs.length === 0 ? (
                <div className="dashboard-card p-10 text-center">
                  ...
                  <h3 className="font-semibold text-gray-500 text-sm">
                    {eligibilityFilterOn ? '내 비자로 지원 가능한 정규직 공고가 없습니다' : '공고가 없습니다'}
                  </h3>
