...
import {
  ...
  AlertTriangle, Crown, Share2, Heart,
} from 'lucide-react';
...
/**
 * 공개 공고 상세 페이지 / Public job detail page
 * 모든 사용자(비로그인 포함) 접근 가능 / Accessible by all users (including guests)
...
export default function PublicJobDetailPage() {
    ...
    const fetchJob = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setError('공고를 불러올 수 없습니다. / Could not load job posting.');
        }
      } catch {
        setError('네트워크 오류가 발생했습니다. / Network error occurred.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleApply = () => {
    ...
    // 로그인 상태면 지원 API 호출 또는 외부 링크 / If logged in, call apply API or external link
    if (job?.externalUrl) {
      window.open(job.externalUrl, '_blank', 'noopener,noreferrer');
    } else if (job?.externalEmail) {
      window.location.href = `mailto:${job.externalEmail}`;
    } else {
  ...
  if (error || !job) return (
    <div className="flex flex-col min-h-screen">
      <Header />
      <div className="flex-grow max-w-4xl mx-auto px-4 py-12 text-center">
        <AlertTriangle className="w-12 h-12 text-gray-300 mx-auto mb-3" />
        <p className="text-gray-500">{error || '공고를 찾을 수 없습니다. / Job posting not found.'}</p>
        <Link href="/" className="text-sm text-blue-600 mt-4 inline-block hover:underline">메인으로 돌아가기</Link>
      </div>
      <Footer />
    </div>
  );

  const status = STATUS_MAP[job.status] || { label: job.status, color: 'bg-gray-100 text-gray-600' };
  const dday = getDday();
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-grow max-w-4xl mx-auto px-4 py-6 w-full">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={() => { if (navigator.share) navigator.share({ title: job.title, url: window.location.href }); }}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-4">
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {job.isUrgent && (
                <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-red-100 text-red-700">급구</span>
              )}
          ...
          {job.company && (
            <p className="text-sm text-gray-600 flex items-center gap-1">
              <Building2 className="w-4 h-4" /> {job.company.brandName || job.company.companyName}
            </p>
          )}
          ...
          <div className="flex items-center gap-6 mt-4 text-sm text-gray-500">
            ...
            <span className="flex items-center gap-1"><Users className="w-4 h-4" /> 지원 {job.applyCount}명</span>
          </div>
        </div>
        ...
        {/* 하단 고정 지원 버튼 / Fixed bottom apply button */}
        <div className="sticky bottom-4 bg-white rounded-xl border border-gray-200 p-4 flex items-center justify-between shadow-lg">
          <div>
            <p className="text-sm font-bold text-gray-900">{formatSalary()}</p>
            <p className="text-xs text-gray-500">{job.company?.brandName || job.company?.companyName}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            {job.status === 'ACTIVE' ? (
              ...
              <span className="px-6 py-2.5 bg-gray-300 text-white text-sm font-medium rounded-lg">마감</span>
            )}
          </div>
        </div>
      </main>
